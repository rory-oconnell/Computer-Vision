...
original_image = cv2.imread("Computer Vision Assignment/Part_II/Tables/Table4.jpg")
hsv_original = cv2.cvtColor(original_image, cv2.COLOR_BGR2HSV)

roi = cv2.imread("Computer Vision Assignment/Part_II/Tables/Table4_BPJ.png")
...
if largest_quad:
    print(f"Largest Quadrilateral Corners: {largest_quad}")
    # Draw only the corners of the largest quadrilateral on the image
    for point in largest_quad:
        cv2.circle(original_image, point, radius=10, color=(255, 0, 0), thickness=-1)  # -1 thickness fills the circle
    cv2.imshow("Largest Quadrilateral Corners", rescaleFrame(original_image, 0.25))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
else:
